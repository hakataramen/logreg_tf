...
def make_graph(vocab_size):
     #モデルパラメータ
     x = tf.placeholder(tf.int32, [None]) # shape=[None]
     one_hot = tf.one_hot(indices=x, depth=vocab_size) # shape=[None, vocab_size]
     embedding = tf.Variable(tf.random_uniform(shape=[vocab_size,50], minval=-1.0, maxval=1.0, dtype=tf.float32)) # shape=[vocab_size, 50]
     word_vecs = tf.matmul(one_hot, embedding) # shape=[None, 50]
     # あるいは
     # word_vecs = tf.nn.embedding_lookup(embedding, x) # one_hotを経由しなくてよくなる
     sentence_vec = tf.reduce_mean(word_vecs, reduction_indices=0) # shape=[50]
     h = tf.reshape(sentence_vec, [1, 50]) # shape=[1,50]
